...
void Map::Update(float deltaTime) {
    // Map doesn't need update logic currently
}

void Map::Draw() {
    // Base draw without shader - draws simple colored cubes
    for (const auto& cube : cubes) {
        DrawCube(cube.position, cube.dimensions.x, cube.dimensions.y, cube.dimensions.z, BLUE);
    }
}

void Map::DrawWithShader(Shader shader, const Vector3& cameraPos) {
    BeginShaderMode(shader);
    
    // Draw cubes with shader
    for (const auto& cube : cubes) {
        DrawCube(cube.position, cube.dimensions.x, cube.dimensions.y, cube.dimensions.z, BLUE);
    }
